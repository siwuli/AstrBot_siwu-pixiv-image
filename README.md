...
- **均衡混排（默认开启，`pixiv_search_balanced`）**：热门搜索不再永远返回同一批顶级图——热门头部保留一张质量位，中段均匀取样，再补一张最新发布且达门槛的新作；热门结果充足时按小时轮换深层页起点（30/60/90），同一关键词反复搜也能换一批，靠后的好图有机会出现；结果很少的词只取第 0 页，不会越界拉空。关闭该配置则严格按收藏热度直取
